...
Navon_L_incon<-function(Navontable){
  ...
  for (i in 5:80) { #skip practice trials
    ...
    assign("vis_L_incon_meanRT", mean(vec,na.rm=TRUE), envir=globalenv())
  }
...
Navon_G_con<-function(Navontable){
  ...
  for (i in 5:80) { #skip practice trials
    ...
    assign("vis_G_con_meanRT", mean(vec,na.rm=TRUE), envir=globalenv())
  }
...
Navon_G_incon<-function(Navontable){
  ...
  for (i in 5:80) { #skip practice trials
    ...
    assign("vis_G_incon_meanRT", mean(vec,na.rm=TRUE), envir=globalenv())
  }
